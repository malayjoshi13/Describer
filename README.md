...
**a) Setting up working environment in "Describe" named folder at Google Drive**

i) Go to the link https://forms.illinois.edu/sec/1713398 and fill the form present there. You will receive Flickr8K data in the gmail id you have provided in the form. Click on both links one by one to download ```Flickr 8k Images``` and ```Flickr 8k Text Data```.

ii) Now as whole training process will happen by help of Google Drive and Google Colab, thus upload/place the zipped ```Flickr8k data``` folder (which you just downloaded) as well as the ```training.ipynb``` file (download it from here https://drive.google.com/file/d/1ZPuK15FFpQt4kPeWRqZpz7qm2EJcmDwh/view?usp=sharing) to your Google Drive in a new folder of  ```Describe```. 

iii) After uploading both folders, unzip them both (i.e ```Flickr 8k Images``` and ```Flickr 8k Text Data``` folders) and from ```Flickr 8k Text Data``` folder, take out ```Flickr_8k.devImages.txt```, ```Flickr_8k.testImages.txt```, ```Flickr_8k.trainImages.txt``` and ```Flickr8k.token.txt``` files. 

iv) After taking out these files, delete ```Flickr 8k Text Data``` folder and rename
```Flickr_8k.devImages.txt``` → ```DevImages.txt```, 
```Flickr_8k.testImages.txt``` → ```TestImages.txt```, 
```Flickr_8k.trainImages.txt``` → ```TrainImages.txt``` and 
```Flickr8k.token.txt``` → ```captions.txt```. 

At the end, your ```Describe``` folder must be looking something like this:
...
iii) Now although we have label encoded training captions in ```(STEP 3.2)```, but as label encoding is not so efficient manner thus by executing ```(STEP 3.4)``` we use Glove embedding. But before executing this step we have to download:
```glove.6B.zip```. For this go to link: https://nlp.stanford.edu/projects/glove/ and there click on the line ```Wikipedia 2014 + Gigaword 5 (6B tokens, 400K vocab, uncased, 50d, 100d, 200d, & 300d vectors, 822 MB download): glove.6B.zip``` 

iv) After downloading, unzip it and take out ```glove.6B.200d.txt``` file and place it in ```Describe``` folder in your Google Drive.

v) Now execute ```(STEP 3.4)``` which creates a dictionary named ```embedding_matrix``` having key->index/numerical encoding of every word in training caption and value->Glove embedding correponding to a particular index/numerical encoding. This dictionary is later saved as ```embedding_matrix.pkl``` file.
<br>
<br>
...
**g) Starting the training process** 
...
i) Now we will initiate training process so that using the structure we designed in ```(STEP 5)``` of model called "training_model", our input data will flow, get observed and memorized by neurons and also updating process of memorized patterns during backpropagation process will happen. We will set some parameters to direct in what way the training process will take place. Therefore, we have asked model to see whole dataset from end-to-end whole 30 times (by setting ```epoch = 30```). In each of these 30 epochs, our model will see whole dataset in piece wise manner, i.e. model has to train itself on 200 batches (this is known as "steps_per_epoch") having 35 elements each.

ii) After all of this process we will finally get our "weights" or the memorization of how each word is related to other word and also to a particular image. We will then save these weights using "training_model.save_weights". 
<br>
<br>

**h) Evaluating the trained model**

i) Using ```(STEP 7.1)``` and ```(STEP 7.2)``` we will extract captions and image-encodings for validation images respectively. 

ii) Using ```(STEP 7.3)``` we will load the saved weights of trained model and then using it in ```(STEP 7.4)``` we predict whole validation captions iteration-by-iteration corresponding to feeded image-encodings and one starting word "startseq". We save these in list "predicted".

iii) On other hand we save each word of each validation captions in list "actual".

iv) Then in ```(STEP 7.5)``` we compare how many words predicted by model actually matches with actual words. On basis of this comparison we calculate BLEU score. BLEU score lies between 0 to 1 and score more towards 1 show higher accuracy of model's pediction.
